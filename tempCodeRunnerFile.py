...
# --- ADDED: Emoji Support Configuration ---
...
def init_database():
    """Initialize database tables with Emoji Support (utf8mb4)"""
    try:
        cur = get_db_connection().cursor()
        
        # Enable utf8mb4 for the connection session
        ...
        # 3. Reports Table (THIS ONE NEEDS EMOJI SUPPORT MOST)
        cur.execute("""
        ...
        cur.close()
        print("Database initialized successfully with Emoji support!")
    except Exception as e:
...
def reports():
    ...
    try:
        ...
        # --- ADDED: FORCE UTF8MB4 SESSION BEFORE INSERT ---
        cur.execute("SET NAMES utf8mb4")
        cur.execute("SET CHARACTER SET utf8mb4")
        # --------------------------------------------------
...
def generate_nlp_report():
    """Generate natural language summary of attendance data with Emoji support"""
    try:
        cur = get_db_connection().cursor()
        ...
        """)
        result = cur.fetchone()
        present_today = result['present_count'] if result else 0
        ...
        # --- CALCULATION & EMOJI LOGIC ---
        # FIX: Check for division by zero
        if total_students > 0:
            daily_percentage = (present_today / total_students) * 100
        else:
            daily_percentage = 0.0

        # Determine overall daily status emoji
        ...
        report_parts.append(f"• Present Today: {present_today}/{total_students} ({daily_percentage:.1f}%) {daily_emoji}")
        report_parts.append("")
        
        if weekly_data:
            report_parts.append("📅 **WEEKLY TREND**")
            for day in weekly_data:
                # Add a mini chart bar using blocks
                count = day['daily_count']
                bar = "█" * count 
                report_parts.append(f"• {day['date']}: {bar} ({count})")
            report_parts.append("")
        ...
        # FIX: Check if total_students > 0 before checking threshold
        if total_students > 0 and (present_today / total_students < 0.8):
        ...
        if low_attendance:
            ...
            for student in low_attendance[:3]:
                report_parts.append(f"  - {student['name']} 📉")
